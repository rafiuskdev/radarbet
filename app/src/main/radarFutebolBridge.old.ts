...
export async function navigateToRfGame(
  ...
  // Recarrega sempre a lista para limpar o estado de popup-blocker entre aberturas
  await rfListPage.goto(RF_URL, { waitUntil: 'load', timeout: 20_000 })
  ...
  // Listener ANTES do clique para não perder o targetcreated
  ...
  // Fuzzy match + clique atómico dentro do evaluate para evitar race condition com DOM updates
  const result = await rfListPage.evaluate((t1: string, t2: string) => {
    ...
    for (const row of Array.from(document.querySelectorAll('tbody tr')) as HTMLTableRowElement[]) {
      ...
      if (!btn) return { ok: false, found: true }
      // Clica atomicamente dentro do evaluate para evitar que o Livewire re-renderize antes do click externo
